...
public class IntegralServiceImpl extends ServiceImpl<IntegralMapper, Integral> implements IntegralService {
    ...
    public void OverdueTimesClear() {
        ...
        //得到总积分
        double totalIntegral = _IntegralMpper.selectList(Wrappers.<Integral>lambdaQuery()
                .eq(Integral::getUserId, userId)).stream().mapToDouble(x -> x.getIntegralValue()).sum();
    ...
    public void SignGiveIntegral() {
        ...
        Long selectCount = _IntegralMpper.selectCount(Wrappers.<Integral>lambdaQuery()
                .eq(Integral::getUserId, userId)
                .eq(Integral::getSource, "每天登录赠送2积分")
        ...
        integral.setSource("每天登录赠送2积分");
        integral.setUserId(userId);
        integral.setTitle("每天登录赠送2积分+" + 2 + "积分");
        saveOrUpdate(integral);
    }
    ...
    public List<Object> GetIntegralConsumeAndGainChart() {
        ...
        for (int i = 0; i < 30; i++) {
            ...
            //得到积分<0的数据综合
            double useIntegral = filterDateList.stream().filter(x -> x.getIntegralValue() < 0).mapToDouble(Integral::getIntegralValue).sum();

            //得到积分>0的数据综合
            double gainIntegral = filterDateList.stream().filter(x -> x.getIntegralValue() > 0).mapToDouble(Integral::getIntegralValue).sum();
