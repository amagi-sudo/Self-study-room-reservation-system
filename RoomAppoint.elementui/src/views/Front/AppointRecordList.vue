  ...
  <div class="app-container margin-top-lg">
      ...
      <div class="tb-body">
        <el-form ref="searchFormRef" :model="searchForm" :inline="true" label-width="120px" :style="`flex:1;`">
          ...
          <el-form-item label="起始时间">
            <el-date-picker v-model="searchForm.BeginTimeRange" type="datetimerange" start-placeholder="开始起始时间"
              end-placeholder="结束起始时间" :default-time="['00:00:00']" value-format="yyyy-MM-dd HH:mm:ss">
            </el-date-picker>
          </el-form-item>
          <el-form-item label="截至时间">
            <el-date-picker v-model="searchForm.EndTimeRange" type="datetimerange" start-placeholder="开始截至时间"
              end-placeholder="结束截至时间" :default-time="['00:00:00']" value-format="yyyy-MM-dd HH:mm:ss">
            </el-date-picker>
          </el-form-item>
        </el-form>
      </div>
    ...
    <PaginationTable ref="PaginationTableId" url="/AppointRecord/List" :column="dataColum" :where="where">
      <template v-slot:Operate="scope">
        <el-button type="primary" size="mini" v-if="scope.row.AppointStatus == 6" @click="ShowEditModal(scope.row.Id)">评
          论</el-button>
...
export default {
  ...
  methods: {
    ...
    async ArrivalClock(id) {
      let { Data } = await this.$PostSigleUpdate(`/AppointRecord/Get`, `/AppointRecord/ArrivalClock`, id, '确定要打卡了吗？', {});
      this.$refs.PaginationTableId.Reload(this.searchForm);
    },

    //取消预约
    async CancelAppoint(id) {
      let { Data } = await this.$PostSigleUpdate(`/AppointRecord/Get`, `/AppointRecord/CancelAppoint`, id, '确定要取消预约吗？', {});
      this.$refs.PaginationTableId.Reload(this.searchForm);
    },

    // 提前结束
    async EarlyEnd(id) {
      let { Data } = await this.$PostSigleUpdate(`/AppointRecord/Get`, `/AppointRecord/EarlyEnd`, id, '确定要提前结束预约吗？', {});
      this.$refs.PaginationTableId.Reload(this.searchForm);
    },
    ...
    */
    async ShowEditModal(Id) {
      try {
        let response = await this.$Post(`/AppointRecord/Get`, { Id: Id });
        if (response.Success) {
        ...
        console.error('获取评论信息时发生错误:', error);
        this.$message.error('获取评论信息时发生错误');
      }
    },
    /**
    * 评论保存
    */
    async CreateOrEditForm() {
      this.$refs.editModalForm.validate(async valid => {
        if (valid) {
          try {
            var response = await this.$Post(`/AppointRecord/SComment`, this.formData);
            if (response.Success) {
            ...
            console.error('评论保存时发生错误:', error);
            this.$message.error('评论保存时发生错误');
          }
        }
      });
    },
